...
public class AggregationService : IAggregationService
{
    ...
    public AggregationService(IEnumerable<IApiProvider> providers, IValueTransformation ValueTransformation, IMemoryCache cache, IStatisticsService statisticsService)
    {
    ...
    public async Task<Result<GetAggregatedDataResponse>> GetAggregatedData(AggregatedDataFilter filter, CancellationToken ct)
    {
        ...
        var tasks = _providers.Select(async provider =>
        {
            ...
            try
            {
                return await provider.GetDataAsync(ct);
            }
            catch (Exception ex)
            {
                isSuccess = false;
        ...
        if (!values.Any())
        {
            return Result<GetAggregatedDataResponse>.Failure("no value was found", ReturnStates.NotFound);
        }
    ...
    public async Task<Result<List<GetRequestsStatistics>>> GetRequestsStatistics(AggregatedDataFilter filter)
    {
